#Specifications - Input images 8x8 , filter size - 3x3 , search range 5
...
inL = read_data_from_file("./left_image_8x8_filein.txt")
inR = read_data_from_file("./right_image_8x8_filein.txt")

#==================================================================================
from symbolic import *

imgW = 8 	#Image_Size width 
imgH = 8 	#Image_Size height 
T_S = imgH*imgW
FILTER_SIZE = 3   #window_Size
...
def Module (f_inL, f_inR ,f_outD) : 
	...
	with ParallelSections ("initial_P1"):
		...
		with ForLoopSection ("R_pixel_in_fifo", "p", 0, imgH * imgW):
      			with LeafSection ("recv_pixel_in_fifo"):
          			...
          			#inp_imgR.writeData (p, f_inR.read ())
		...
		with LeafSection ("leaf_le4"):
			for cl in range(SEARCH_RANGE):
	...
	with SerialSections ("initial_P1"):
		with ForLoopSection ("row_iter", "row", 0, imgH):
			with ForLoopSection ("col_iter", "col", 0, imgW-2):
	    			...
	    			with LeafSection ("leaf_le_pp2_"+ str (row)+"_"+str (col)):
	...
	#===================================================================================================================================    			
	    			...
	    			#with LeafSection ("leaf_le_pp99"):
	    			#	display("===========================================================")
	    			#	for ii in range(FILTER_SIZE):
	    			#		display("right_colL[%d] is %d",ii,right_colL[ii])
	    			#		display("right_colR[%d] is %d",ii,right_colR[ii])	
	    			
	    			with LeafSection ("leaf_le_pp3_"+ str (row)+"_"+str (col)):
	    				for ii in range(FILTER_SIZE):
						for jj in range(FILTER_SIZE - 1):
							Assignment(windowL[(jj+(ii*FILTER_SIZE))] , windowL[((jj+1)+(ii*FILTER_SIZE))])
							Assignment(windowR[(jj+(ii*FILTER_SIZE))] , windowR[((jj+1)+(ii*FILTER_SIZE))])
	    			...
	    			with LeafSection ("leaf_le_pp4_"+ str (row)+"_"+str (col)):
					for ii in range(FILTER_SIZE):
				...
				with SerialSections ("disp_output"):
		    			with ForLoopSection ("d_iter", "d", 0, (SEARCH_RANGE-2)):
		    				'''
		    				with LeafSection ("leaf_le_disp100"+ str (row)+"_"+str (col)+"_"+str (d)):
		    					#with Leaf:
		    					for ii in range(FILTER_SIZE):
			    					for jj in range(FILTER_SIZE):
						...
						with LeafSection ("leaf_le_disp1"):
		    					Assignment(cost_row[(d+1)+(col*SEARCH_RANGE)] , left_d) #dup1
		    				...
		    				with LeafSection ("leaf_le_disp2"):
		    					...
		    					for ii in range(N):
		    						...
		    						#if(((windowL[ii] > x_ham) & (windowR_shift[ii] > y_ham)) | ((windowL[ii] <= x_ham) & (windowR_shift[ii] <= y_ham))):
		    							#display("abs_sum")
		    						#	abs_sum = abs_sum
		    						#else :
		    						#	abs_sum = abs_sum+1
		    				...
		    				#with LeafSection:
		    				#	display("obtained cost[0] top_left_d=%d,top_left_d_prev=%d,top_left_d_forw=%d,min_cost_row[col-1]=%d",cap[0], cap[1], cap[2], cap[3])
		    				#	display("obtained cost[1] top_d=%d,top_d_prev=%d,top_d_forw=%d,min_cost_row[col]=%d",cap[4], cap[5], cap[6], cap[7])
		    				#	display("obtained cost[2] top_right_d=%d,top_right_d_prev=%d,top_right_d_forw=%d,min_cost_row[col+1]=%d",cap[8], cap[9], cap[10], cap[11])
		    				#	display("obtained cost[3] left_d=%d,left_d_prev=%d,left_d_forw=%d,min_cost_left=%d",cap[12], cap[13], cap[14], cap[15])	
		    				
		    				with LeafSection ("leaf_le_disp3"):		#computing directio path costs in a cycle (for inside with leaf)
			    				for i in range(4):
		    				...
		    				with LeafSection ("leaf_le_disp4"+ str (row)+"_"+str (col)+"_"+str (d)):
			    				Assignment(index, (((col-d)-FILTER_SIZE)+1))
			    				#display("index value is  %d",index)
			    				for ii in range(FILTER_SIZE):
			    					if(index > 65500):
		    				...
		    				with LeafSection ("leaf_le_disp45"+ str (row)+"_"+str (col)+"_"+str (d)):	
		    					Assignment(path_cost , (cost[0] + cost[1] + cost[2] + cost[3]))
		    					path_var =  ham_dist[d+1]
		    					sum_cost = ((path_cost) >> 2) + path_var
		    					#sum_cost = ( ham_dist[d+1] + path_var)
		    				#	display("d= %d , row= %d, col= %d, cost[0] %d, cost[1] =%d, cost[2] =%d,cost[3] =%d, path_cost is %d,ham_dist[d+1] =%d ",d+1,row,col+1,cost[0] ,cost[1],  cost[2], cost[3],path_cost,ham_dist[d+1])
		    					
		    					if (sum_cost > (MAX_COST + P2)):
		    						Assignment(sum_cost , MAX_COST + P2)
		    					Assignment(cost_left[d+1] ,  sum_cost)
		    					#display("sum_cost is %d", sum_cost)
		    					if(sum_cost < min_sum_cost):
			    					Assignment(min_sum_cost , sum_cost)
			    					Assignment(winning_disp , d)
				
			    		with LeafSection ("leaf_le_out"+ str (row)+"_"+str (col)):
